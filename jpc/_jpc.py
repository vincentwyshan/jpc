...
class BaseHandler(object):
    ...
    def _get_method(self, name):
        ...
        if '.' in name:
            m = self._get_subhandler_method(name)
        else:
    ...
    def _get_subhandler_method(self, name):
        '''
        Vincent add this function to support sub-handler.
        '''
        names = name.split('.')
        tmp = self
        for n in names:
    ...
    try:    
        ...
        if id is not None:
            return json.dumps({'result': None, 'error':
                repr(sys.exc_info()[1]), 'id': id}, cls=JsonEncoder)
...
class Proxy(object):
    ...
    def __getattr__(self, name):
        """Return proxy version of method. support sub-handler"""
        class _Proxy(object):
            ...
            def __getattr__(self, name):
                if name.startswith('_'):
                    ...
                    return _Proxy('%s.%s'%(self.name, name), self.call)

            def __call__(self, *args, **kwarg):
                if name.startswith('_'):
                    return super(_Proxy, self).__call__(*args, **kwarg)
                else:
                    return self.call._proxy(False, self.name, args, kwarg)
